...
code Get-Addr ( addr1, size, count -- addr2 )
	...
	next c;
\
chdir \programming\Win32Forth\include
include string.f
include file-emit.f
\
chdir \programming\Win32Forth\proj\crossx
include win-crossx-20.h \ Header File
\
defer menubegin#
...
\
\ ********************************************************************************
25 hash-size CHANNEL hash{ ( Hash Table )
...
:M On_Done:
		   ...
		   Cxref-Mem-deallot
                   ...
                   On_Done: super 0
		   turnkey? if bye else bye then \ terminate application
;M
\
...
: Crossref
	...
	file-flag \ If file is open process
		  ...
		  if  \ file open successful
			...
			s" Crossxv3.wrk" delete-file drop \ Delete work file
...
\	s" Messages loaded" type cr
      cxref-mem-allot
\	s" cxref-mem-allot Completed" type cr
\
   start: xref-Window Winpause
\	s" Starting Windows" type cr
   ApplicationBar SetMenuBar: xref-Window Winpause
\	s" ApplicationBar " type cr
\   gethandle: xref-Window
\	s" FileLocate -- " type
\   Start: filelocate
\	s" Done" type cr
\   file-flag enable: MenuBegin
\   0 Cxref-fileopen
   Paint: xref-Window Winpause
   file-flag enable: MenuBegin Winpause
   false enable: MenuClose-display#
...
[IF]
	...
	chdir c:\Programming\Win32Forth\proj\Crossx
	' cxref turnkey Crossref-21.exe
	s" Reference.ico" s" Crossref-21.exe" AddAppIcon
[ELSE]
\ ***************************************************************************************
: CXR       ( -- )                  \ start running the program
		( Start:  xref-Window ) cxref ;
\ ***************************************************************************************
[THEN]
\
\s
