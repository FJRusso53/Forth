\ $Id: WinEd.f,v 1.1 2004/12/14 23:54:02 alex_mcdonald Exp $
((
    ***********************************************************************
     *                                                                   *
    ...
    ***********************************************************************
        ...
        NOTE: At Tom Zimmer's request, WinView was re-named WinEd
  ...
  To recompile this file see the notes at the end of this file.
))
...
\ add the WinEd folder's to our path list
...
\ changed June 1st, 2003 - 10:08 dbu
\ to wait until Forth is loaded
: StartUpForth  { \ file$ -- } \ Start forth console
        ...
        \ adjust count of Forths currently running
        -1 ed-forth-count +! ;  \ SF-RequestID 745393 - fixed June 8th, 2003 - 12:59 dbu
...
: +cursor-line ( n1 -- fl)  \ changeline  fl:true=>update scrn
  dup cursor-line line#>indx dup>r +
...
: put-cursor-line ( -- )  \ what does this one do?
...
\ 7 --- Drawing into the Image DC   What does DC stand for? -------------------
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

: moveto        ( x y -- )  \ Move to the x y position in the editor jp???
                imageDC MoveTo: [ ] ;

: lineto        ( x y -- )   \ How does this differ from the above   jp???
                imageDC LineTo: [ ] ;

: line          ( x1 y1 x2 y2 -- ) \ move the line from xy to x2y2   jp??
                2swap moveto lineto ;

: line-color    ( color_object -- )  \ what does this do jp???
                ?ColorCheck
                imageDC LineColor: [ ] ;
...
\ 9     Simple Statusbar Class
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

\ now in WinEd_Statbar.f
\ August 17th, 2003 - 17:18 dbu
needs WinED_Statbar.f
...
\ changed September 20th, 2003 - 19:32 dbu
\ Colorization now works for *.f *.fr and *.seq files
: Forth-Source? ( -- flag )                      \ Is current file *.F ?
        cur-filename count ".EXT-ONLY" 2dup upper
        2dup s" .F"   compare 0= -rot ( f a n )
        2dup s" .FR"  compare 0= -rot ( f f a n )
             s" .SEQ" compare 0=      ( f f f )
             OR OR ;                  ( f )
...
: typetext      ( a n -- ) \ what does this do  jp???
...
: "PutTextOut   ( a1 n1 -- )  \ Need a short comment here  jp???
...
: TextTo';'     ( a1 n1 -- a2 n2 )      \ comment please jp???
        2dup ';' scan                   \ find ';'
        2dup 1 /string 2>r              \ skip ';' and save following string
        nip - number? 2drop             \ extracted number string
...
: Underline     ( -- )                  \ put out an underline on screen jp??
...
: /Bold         ( -- )
        Handle: vFont SetFont: dc ;     \ cancel bold font
    ...
    IF  TRUE to alreadyPainting
        ...
        BEGIN
            ...
            IF  margin-check
                ...
                ( Prnter settings )
                IF  right-edge 0=
                    ( Declare printer settings if not in console )
                    IF  page-lines?
                ...
                THEN
                window-lmargin 0>
                ( Display page breaks, and work with the printer )
                IF  linesColor   LineColor: dc   \ display page breaks
                    ...
                    line-cur -
                    ( Output text in the correct font )
                    BEGIN dup window-lines <
                    WHILE dup  ( MIDDLE OF LOOP )
                        page-lines? and
                        self ConsoleWindow <> and
                        \ show page lines if not console
                        ( Show the page number on break line )
                        IF  window-Lmargin
                        ...
                        lend-len
                        ( Output text in the right character hight or . . )
                        IF  1+
                        THEN
    ( factor me )         0 <# # 2dup #> 2dup GetTextExtent: dc drop
           ...
           ( 0 )            r@     CharHeight: self *
                        ...
                        IF  <# # 2dup #> 2dup GetTextExtent: dc drop
                            window-Lmargin 5 - swap - 1 max
           ( 1 )            r@ 1 - CharHeight: self *
                            2swap TextOut: dc
                        THEN
                          over 0> r@ 1 - 0> and
                        IF  <# # 2dup #> 2dup GetTextExtent: dc drop
                            window-Lmargin 5 - swap - 1 max
           ( 2 )            r@ 2 - CharHeight: self *
                            2swap TextOut: dc
                        THEN
                          over 0> r@ 2 - 0> and
                        IF  <# # 2dup #> 2dup GetTextExtent: dc drop
                            window-Lmargin 5 - swap - 1 max
           ( 3 )            r@ 3 - CharHeight: self *
                            2swap TextOut: dc
                        THEN
...
: set-console-height ( -- )
                GetSize: EditorWindow nip drag-barV - to console-height ;
...
: unminimize-EditWindow ( -- )
                ...
                IF      GetSize: EditorWindow nip 2/ to drag-barV
                        set-console-height
...
: GetForthWindow ( -- ) \ get the handle of the forth console window
        ...
        &GetForthWindowCallback rel>abs
...
: wined_win32forth-message ( lParam wParam -- ) \ send message to forth console
        show-console? \ are we using WinEd's internal console window?
        if   \ yes, so use the org. win32forth-message function witch send's the
             \ message by broadcasting it to all windows.
             _win32forth-message
        else \ check if we have a valid window handle to the forth console
             hWndForthWindow call IsWindow not
...
\ 17    WinEd about dialog, copied from the Forth About Dialog
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
create about-head
        z," WinEd (Public Domain January 1995)"
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
  ...
  FileNewDialog ViewText "Load Text File" "Forth Files (*.f,*.seq)|*.f;*.seq|All Files (*.*)|*.*"
  FileSaveDialog SaveText "Save Text File" "Forth Files (*.f)|*.f|All Files (*.*)|*.*|"
    ...
    IF
        ...
        IF  drop                            \ rda jp 01/27/2002  DROP added
            ...
            IF    drop                      \ can't, then strip off path
                  ...
                  "CLIP" open$ place
( dbu )           path-ptr to org-path-ptr    \ save search path
( dbu )           MAXSTRING localAlloc: path$ \ set default search path
( dbu )           path$ to path-ptr program-path-init
                  open$ count "path-file    \ search through the Forth path
                  drop "CLIP" open$ place
( dbu )           org-path-ptr to path-ptr  \ restore search path
...
: +hyper        ( n1 -- )       \ next hyper
        ...
        ELSE    drop    \ May 5th, 1998 tjz added from bjg
        THEN    ;
...
: show-command-help     ( -- )
        ...
        z" WinEd DOS Commandline Help" rel>abs
...
: open-initial-file ( -- ) \ open the first file on startup
        ...
        file-to-edit$ count 0= swap c@ 0= or and   \ and no file specified
        ...
        IF      open-previous? 0=               \ open the previous file?
                ...
                THEN
        ELSE    unminimize-EditWindow
                file-to-edit$ count 0= swap c@ 0= or 0= \ is there a file to open
...
\ 38    Cause text command string to be interpreted.
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
: "Forth        ( a1 n1 -- )
                ...
                [else] 0
                [then]
                if   source 2>r >in @ >r (source) 2! >in off
                     _interpret r> >in ! 2r> (source) 2!
                else ?ConsoleReady
                     if   bounds
                          ?DO  I c@ rem-sendkey
...
\ 33    Process a carriage return from user
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
\ 42    Search Files for text dialog Object
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
: get-options   { \ number$ -- }
        ...
        IF      show-console?                   \  we want the console
                IF      StartConsole: EditorWindow
                ELSE    CloseConsole: EditorWindow
...
\ 48    Debug a Forth application
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

fload EdDebug.f

0 value inq-running?
:noname ( -- )          \ inquire for the value of a data item
        ...
        IF      TRUE to inq-running?
                ...
                IF      ed-inquire EditorWindow Start: InquireDlg
                ELSE    TRUE
                THEN
...
: view-key      ( -- c1 )
                ...
                IF      Paint: ConsoleWindow
                        ReTitle: EditorWindow
                THEN    _mkey [ also HIDDEN ] log-also [ previous ] ;
...
: Compile-File ( addr n -- ) { \ load$ file$ ismerged -- } \ compile a file
        ...
        [if]  (wineddbg)
        [else] 0
        [then] dup to ismerged 0=
       if
        show-console? 0=
        IF
             FindForthWindow not  \ if no forth running,
             IF StartUpForth THEN \ then start one
       ...
       then
        ...
        ismerged
        if focus-console 2r> 2drop then ;       \ go to console (doesn't wrk)
...
\ 56    These are now in WinEd_Toolbar.f   September 10th, 2003 - 13:09 dbu
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

\       Enhance the ToolBar class for WinEd
\       Define the ToolBar for the application

needs WinEd_Toolbar.f


\ 57    These are now int WinnEd_Toolbar.f  September 10th, 2003 - 13:09 dbu
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ 58    Define the Editor popup menu for the application
\ 59    Define the Console popup menu for the application
\ 60    Define the ToolBar popup menus for the application
\ 61    RECENT FILES menu
\ 62    Define the menubar for the application
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
\		101 appInst Call LoadIcon
		s" res\WinEd.ico" Prepend<home>\ LoadIconFile
		;M
...
:M SetMultiStatus: ( a1 n1 a2 n1 -- ) \ set the current text on statusbar
        ...
        SetMulti: WinEdStatusbar \ set statusbar to show multiparts

        s"  Column: "                                   StatusBuf  place
        cursor-col 1+  0 <# #s #>                       StatusBuf +place
        s"  of "                                        StatusBuf +place
        max-cols       0 <# #s #>                       StatusBuf +place
        StatusBuf +null StatusBuf 1+ 0 SetText: WinEdStatusbar

        s"  Line: "                                     StatusBuf  place
        cursor-line 1+ 0 (ud,.)                         StatusBuf +place
        s"  of "                                        StatusBuf +place
        file-lines     0 (ud,.)                         StatusBuf +place
        StatusBuf +null StatusBuf 1+ 1 SetText: WinEdStatusbar
        ...
        THEN
        StatusBuf +null StatusBuf 1+ 2 SetText: WinEdStatusbar

        s"  Forth Instances: "                          StatusBuf  place
        ed-forth-count @ 0 (d.)                         StatusBuf +place
        StatusBuf +null StatusBuf 1+ 3 SetText: WinEdStatusbar

        overstrike @
        if   s"  <OvrStrk>"
        else s"  <Insert>" then                         StatusBuf  place
        StatusBuf +null StatusBuf 1+ 4 SetText: WinEdStatusbar
        ;M

:M SetSimpleStatus: ( a1 n1 a2 n1 -- ) \ set the current text on statusbar

        SetSingle: WinEdStatusbar \ set statusbar to show single-part
...
:M StartToolBar: ( -- )
        ...
        IF      TRUE to tool-bar?
                GetSize: EditorWindow nip 2/ to drag-barV
                set-console-height
                self Start: WinEdToolbar          \ then startup child window
                LoadFindStrings
                Refresh: self
...
\ 59    Support for Drag and Drop files from File Manager to WinEd
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
:noname { wParam -- }
        ...
        refresh-screen
        RefreshCursor: DocWindow
        EditMode: WinEdToolbar ; is remote-open
...
:noname ( -- )
        ...
        IF      cursor-line screen-rows 2 -
                VPosition: edit-window
        THEN
        refresh-screen
        RefreshCursor: DocWindow
        EditMode: WinEdToolbar ; is remote-word
...
\ 62    Need a name for this paragraph
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
\ 63    Save and restore default settings for WinEd
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

\ new May 24th, 2003 - 8:55 dbu
\ changed July 20th, 2003 - 18:30 dbu
: save-previous-file { n \ buff$ buff1$ -- } \ save the filename and cursorposition off the active file in registry
...
\ changed July 20th, 2003 - 18:30 dbu
: save-previous-files ( -- ) \ save the filenames and cursorpositions off the active files in registry
...
: load-defaults ( -- )
    ...
    s" ForthWindow"    "GetDefaultValue IF 0<> to show-console?  ELSE drop THEN
    ...
    s" SearchPath"   "GetDefault -IF 2dup "CLIP" path-ptr  place THEN 2drop
    ...
    s" SplitHeight"  "GetDefaultValue IF to console-height    ELSE drop THEN
    s" SaveLines"    "GetDefaultValue IF to console-savelines ELSE drop THEN
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
      ...
      'S'     +k_control +k_shift   OF      [ also HIDDEN ] Start/Stop-macro [ previous ] ENDOF
...
\      k_F1    ( rbs )               OF      s" htm\STARTUP.TXT" Prepend<home>\ "browse    ENDOF
      k_F1    ( rbs )               OF      s" htm\p-getstart.htm" Prepend<home>\ "browse    ENDOF
      ...
      k_F5                          OF      [ also HIDDEN ] replay-macro [ previous ] ENDOF
...
: WinEd-init  ( -- )
      ...
      ['] wined_win32forth-message is win32forth-message
      ...
      -1 -1 prevhst 2! -1 -1 prevhed 2! \ init HIGHLIGHT-CASE-TOGGLE (Strg+U)
...
: WINED       ( -- )
        ...
        command-options
        Start: FrameWindow
        init-shared-type \ setup shared memory
        floating-bar?
        IF      Float: WinEdToolbar
        THEN
        load-more-defaults
...
CreateTurnkey [if]
        ...
        0 0 ' wined APPLICATION WinEd   \ save WinEd.exe
...
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
\ 79    Done Items and comments. Latest on bottom. Ctrl+Shift+V inserts date
\ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

Gotchas
To compile the editor all instances of Win32Forth must be closed

Completed items
The list of files window on the left of the editor is retained across sessions
))
