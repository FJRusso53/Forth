\ Encrypt.F
\ Frank J. Russo
\ Version 2.2  Support for Folder - Directory - Sub-Diretory Processing
\ Date 151118
\
...
\  check for bypass
  ...
  If    \ Do they match ?
	...
	0 0 infile-ptr REPOSITION-FILE drop \ reset file pointer to begining of file Version 1.
        \ locate * in stream +1 = STP
	  infile-buffer-ptr 60 Star$ InstrB + 1+ dup
          \ locate idm-exit in stream = EDP
	  infile-buffer-ptr 60 IDM_Exit InstrB +
          \ len = EDP - STP
          swap - Pw-len !
...
if
 0 Do Dup Dup @ 31 or 0F and swap C! 1+ loop drop  \ Encrypt the password
then
\
lock-flag @
if
  password pw-len @ msg2a swap cmove \ move the encrypted pw to message field for output
...
: Enc-Proc5
	...
	if
	  ...
	  True to version2
	  infile-buffer-ptr 60 IDM_Exit InstrB \ Locate IDM_Exit in the first 64 characters of the input stream
	  1+ to inb-offset \ save location after the IDM_Exit
	  drop
...
\
				...
				then
                        then
\
		     dir-path folder-offset + c@ 0=
\
                    Until           \  Added 151060 for Folder Support
\
...
' ENCRYPT turnkey Encrypt-XX               \ create - Encrypt2.exe
        s" \Win32Forth\proj\Encrypt\Lock.ico" s" \Win32Forth\proj\Encrypt\Encrypt-XX.exe" AddAppIcon
        1 pause-seconds bye
