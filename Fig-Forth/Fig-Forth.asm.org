...
Fig-Forth.asm
 FJRusso Vs 2.3  
 Sunday, December 17, 2023 12:29:06 PM
...
ypos DD 0
; -----------------------------
Msg1 DB  '        Fig-Forth vs 2.3', 13, 10, 0h
Msg1A DB '            FJRusso', 13, 10, 0h
Msg2 DB  '    Saturday, December 09 2023' , 13, 10, 0h
...
JMP do_DOCOL
 ...
 DD do_TIB, do_BL, do_WORD                          ; INPUT BUFFER, SEARCH DELIMITER, PARSE INPUT STRING
...
NEXTC
;
_BL:                    ; SHOULD BE DEFINED AS A FORTH CONSTANT
DD _STATE
DD do_BL
DB 2, 'BL'
do_BL:
...
JMP do_DOCOL
DD do_DECIMAL
DD do_WDCOUNT, do_DCW, do_FETCH, do_PAD
...
do_ABORT@:
MOV ABORTFLAG, -1
LEA EDI, Forth_Thread + 4 ; Load DI with Forth_Thread
NEXTC
;
_COLON:         ; VERIFIED
DD _ABORT@      ; Link File Addess (LFA) Pointer to previous words
DD do_COLON   ; CFA - Code Field Address
DB 1, ':'       ; NFA - NAME FIELD
do_COLON:
DD do_NEWHEADER, do_GCFA, do_DOCOL, do_NFA, do_LIT, 4, do_MINUS,  do_STORE
DD do_STATE, do_ON
;
_SCRINIT:
DD _COLON
...
do_SCRINIT:
    ...
    invoke udw2str, TV1, HLD   ; Convert unsigned DWORD to string
    invoke StdOut,  HLD
    ADD HLD, 16
    invoke StdOut, ADDR Msg15
    invoke StdOut, ADDR Msg13
    invoke udw2str, TV2, HLD   ; Convert unsigned DWORD to string
    invoke StdOut,  HLD
    ADD HLD, 16
    invoke StdOut, ADDR Msg15
    invoke StdOut, ADDR Msg14
    invoke udw2str, TV3, HLD   ; Convert unsigned DWORD to string
    invoke StdOut,  HLD
    ADD HLD, 16
...
NEXTC
;
NEWNUMB: ; ( ADDR -- N F )  ONLY WORKS FOR INTEGERS (+/-)       ; VERIFIED
DD NUMINIT
DD do_NEWNUMB
DB 7,'NEWNUMB'
do_NEWNUMB:
...
ADD EAX, TV$
MOV BYTE PTR [EAX], 0
invoke atol, TV$ ; CONVERT ASCII STRING TO NUMBER (EAX)
PUSH EAX
PUSH -1
NEXTC
;
...
@@1HV:
 ...
 NEXTC
;
EXECUTE:   ;( cfa -- )  execute a Forth word, given its cfa  ; VERIFIED
DD HASHV
DD do_EXECUTE
DB 7, 'EXECUTE'
do_EXECUTE:
...
DB 3, 'MS@'
do_MS:
invoke GetLocalTime, ADDR _SYSTIME
LEA EBX, _SYSTIME
MOV ECX, 60
ADD EBX, 8
MOV EAX, 0
...
;
_FILEOPEN:         ; (ADDR, MODE -- IOR, F )        VERIFIED 231214
DD _CMAX
DD do_FILEOPEN
DB 9, 'OPEN-FILE'
do_FILEOPEN:
...
do_FPARMSRW:
   ...
   RET ; NEXTC
;
_READFILE:          ; (  BUFFER, #BYTES, HNDL  -- ior )  ior = 0 = success     VERIFIED 231214
DD _FPARMSRW
DD do_READFILE
DB 9, 'READ-FILE'
do_READFILE:
...
DB 8, 'ALLOCATE'
do_ALLOCATE:
POP TV1     ; SIZE REQUESTED
invoke GlobalAlloc,0,TV1            ; GMEM_MOVEABLE=0 or GMEM_ZEROINIT=2
mov _hMemory,eax
.if eax!=0
invoke GlobalLock, _hMemory
mov pMem,eax
PUSH pMem
CALL GetLastError
PUSH EAX
.else
invoke StdOut,ADDR Msg18
PUSH 0
.endif
NEXTC
;
...
do_FREE:
POP TV1         ; pMem
invoke GlobalUnlock, TV1        ; pMem
invoke GlobalFree, TV1          ; pMem
NEXTC
;
...
PUSH $ + 10
JMP do_DOCOL 
DD do_PAD, do_dwtoa             ; CONVERT n TO ASCII
...
DD do_VARHEAD      ;DD do_DOVAR DD do_COMPILECOMMA
...
NEXTC
;
_CONSTANT:  ; ( n "name" -- )    create a constant (unchangeable) value    VERIFIED
DD _CONHEAD
DD do_CONSTANT
DB 8, 'CONSTANT'
do_CONSTANT: 
...
NEXTC
;
_VARIABLE:      ; ( "name" -- )      create a variable (changeable) value     VERIFIED 231201
DD _VARHEAD
DD do_VARIABLE
DB 8, 'VARIABLE'
do_VARIABLE:
...
DD do_SEMI
;
_VALUE:         ; ( n "name" -- )  \ create a self fetching changeable value  VERIFIED 231201
DD _VARIABLE
DD do_VALUE
DB 5, 'VALUE'
do_VALUE:
PUSH $ + 10
JMP do_DOCOL
DD do_TIB, do_BL, do_WORD, do_COUNT
DD do_NEWHEADER                                                       ; 'n TO value-name' will change a value
DD do_GCFA, do_DOVALUE, do_NFA, do_LIT, 4, do_MINUS,  do_STORE        ; DOVALUE COMPILE
...
DD do_PAD, do_dwtoa, do_PAD, do_PUSH0               ; invoke dwtoa, dwValue:DWORD, lpBuffer:DWORD 
DD do_STDOUT                                        ; DISPLAY ASCII TEXT IN PAD
DD do_LIT, 93, do_EMIT, do_SPACE, do_EMIT           ; do_DOTP, "]"
...
PUSH $ + 10
JMP do_DOCOL
DD do_TIB, do_ZCOUNT ; ( TIB -- TIB LEN)
...
PUSH $ + 10
JMP do_DOCOL
DD do_TIB, do_BL, do_WORD, do_COUNT, do_FIND ; LOCATE LFA OF WORD TO REMOVE
...
DB 4,'KEY?'
do_KEYQ:
Invoke GetNumberOfConsoleInputEvents, 0, KEYIN
NEXTC
;
...
PUSH $ + 10
JMP do_DOCOL
DD do_TIB, do_BL, do_WORD, do_COUNT, do_FIND
DD do_SEMI
;
_TO:                        ; VERIFIED 231201
DD _TICK 
...
DD do_ZCQ             ;      HERE [CHAR] " PARSE Z", 0 C, ALIGN ZCOUNT \N->CRLF
DB 3,'Z,"'        
do_ZCQ:
PUSH $ + 10
JMP do_DOCOL
DD do_HERE, do_LIT, '"', do_PARSE
; DD do_ZQC, do_PUSH0, 
...
DD do_SEMI
;
_WORDS:     ; listing of dictionary words          VERIFIED
DD _ANEW
DD do_WORDS
DB 5, 'WORDS'
do_WORDS:
...
DD do_BEGIN@
DD do_DUP, do_LIT, 8, do_PLUS, do_DUP, do_PUSH1, do_PLUS, do_SWAP, do_CFETCH, do_DUP, do_TOR ; SAVE LENGTH
...
DD do_SEMI
;
_DUMP:         ; ( ADDR, N --  )  VERIFIED 231215
DD _CTYPE
DD do_DUMP
DB 4, 'DUMP'
do_DUMP:
POP TV1         ;  # OF BYTES TO DUMP
POP TV2         ; ADDRESS TO DUMP
PUSH $ + 10
JMP do_DOCOL
DD do_CR
@DLOOP1:
