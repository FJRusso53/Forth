...
Fig-Forth.asm
 FJRusso Vs 2.54  
 Thursday, January 18, 2024 2:44:21 PM  
...
2 PROC SetEndOfFile
 ...
 invoke Free, proc public pv:DWORD                 : DONE
 invoke GlobalUnlock, pMem                         : DONE
 invoke GlobalFree, hMemory                        : DONE
 invoke GlobalAlloc,0,TV1                          ; GMEM_MOVEABLE=0 or GMEM_ZEROINIT=2   DONE
 invoke GlobalLock, hMemory                        ; DONE
...
;
    ...
    .LISTALL
      	invoke Main
;
    LEA EDI, Forth_Thread 			; Load EDI with Forth_Thread
	NEXTC
;
_TI1:
DD 0000
...
JMP do_DOCOL
 ...
 DD S_LEN, @, S_IN, @                            ; S_LEN _IN - 1 > 
 DD MINUS, PUSH1, GT@
 DD UNTIL@ 
 DD PUSH0, S_IN, STORE, QUIT                     ; 0 S_IN !  DISPLAY 'ok'
 DD QBYE
 DD UNTIL@ 
 ; DD SEMI
 DD @@EXIT
...
NEXTC
;
_XYLOCATE:                      ; ( X, Y, -- )  SCREEN POSITIONING  VERIFIED 240116
DD _NULL
XYLOCATE:
DD do_XYLOCATE
DB 0, 8, 'XYLOCATE'
...
PUSH ESP
POP EAX
MOV [S0+8], EAX                     ; Saving Stack Pointer Base 
...
do_COLDIIT:
    ...
    SUB EAX, Forth_Thread           ; Forth_Thread
...
NEXTC
;
_SCRINIT:                           ; VERIFIED 240116
DD _COLDINIT
SCRINIT:
DD do_SCRINIT                       ; do_COLN          
DB 0, 7, 'SCRINIT'
do_SCRINIT:
PUSH $ + 10
JMP do_DOCOL
DD MSGLOAD, COLDINIT                ; LOADS MESSAGE FLE INTO MEMORY AND BUILDS INDEX ARRAY
DD LIT, 1250, PAUSE@
DD CLS, PUSH0, PUSH0, XYLOCATE
DD LIT, 2, MSGOUT, CR
DD LIT, 3, MSGOUT, CR
DD LIT, 4, MSGOUT, CR
DD LIT, 14, MSGOUT, CR
DD LIT, 15, MSGOUT, CR
DD LIT, 16, MSGOUT, CR, CR
DD LIT, 19, MSGOUT, CR
DD LIT, 3, LIT, 15, XYLOCATE               
DD HLD, @, TV1, @, UDW2STR, HLD, @, ZCOUNT, TYPE@       ; Convert unsigned DWORD to string and display
DD LIT, 4, LIT, 14, XYLOCATE 
DD HLD, @, TV2, @, UDW2STR, HLD, @, ZCOUNT, TYPE@       ; Convert unsigned DWORD to string and display
DD LIT, 5, LIT, 14, XYLOCATE 
DD HLD, @, TV3, @, UDW2STR, HLD, @, ZCOUNT, TYPE@       ; Convert unsigned DWORD to string and display
DD LIT, 3, LIT, 21, XYLOCATE, LIT, 17, MSGOUT
DD LIT, 4, LIT, 21, XYLOCATE, LIT, 17, MSGOUT
DD LIT, 5, LIT, 21, XYLOCATE, LIT, 18, MSGOUT
DD LIT, 8, PUSH0, XYLOCATE, LIT, 26, MSGOUT 
DD LIT, 9, PUSH0, XYLOCATE, LIT, 27, MSGOUT
DD LIT, 10, PUSH0, XYLOCATE, LIT, 28, MSGOUT
DD SEMI
;
...
do_CPLUSSTORE: 
    ...
    NEXTC
 ; 
_WFETCH:      ; ( a1 -- w1 )     ;    fetch the word ; (16bit) w1 from address a1
DD _CPLUSSTORE
WFETCH:
DD do_WFETCH
DB 0, 2, "W@"
do_WFETCH: 
    ...
    NEXTC
 ; 
_SWFETCH:     ; ( a1 -- w1 )     ;    fetch and sign extend the word ; 
DD _WFETCH 
SWFETCH:
DD do_SWFETCH
DB 0, 3, "SW@"
do_SWFETCH: 
    ...
    NEXTC
 ; 
_WSTORE:      ; ( w1 a1 -- )     ;    store word ; (16bit) w1 into address a1
DD _SWFETCH 
WSTORE:
DD do_WSTORE
DB 0, 2, "W!"
do_WSTORE: 
    ...
    NEXTC
 ; 
_WPLUSSTORE:     ; ( w1 a1 -- )     ;    add word ; (16bit) w1 to the contents of address a1
DD _WSTORE
WPLUSSTORE:
DD do_WPLUSSTORE
DB 0, 3, "W+!"
...
do_U2DIVIDE: 
    ...
    NEXTC
 ; 
_1PLUS:     ; ( n1 -- n2 )  add one to n1   ; VERIFIED 20114
DD U2DIVIDE
PLUS1:
DD do_1PLUS
DB 0, 2, "1+"
do_1PLUS: 
    ...
    NEXTC
 ; 
 _2PLUS:     ; ( n1 -- n2 )  add one to n1   ; VERIFIED 240114
DD _1PLUS
PLUS2:
DD do_2PLUS
DB 0, 2, "2+"
do_2PLUS: 
    ...
    NEXTC
 ;
_1MINUS:      ; ( n1 -- n2 )  subtract one from n1    ; VERIFIED 240114
DD _2PLUS
MINUS1:
DD do_1MINUS
DB 0, 2, "1-"
...
do_LROT32: 
 ...
 push ebx
    NEXTC
 ; 
_GETADDR:  	; ( addr1, size, count -- addr2 )  ;  Get address of an array element
DD LROT32   ; Address of array base, size of element (row), count = row #
DD do_GETADDR
DB 0, 7, "GETADDR"
do_GETADDR: 
    POP EAX 
    ...
    NEXTC
;
_LIT:           ; ( -- n )  COMPILATION WORD ONLY                            VERIFIED 240122
DD _GETADDR     ; push the literal value following LIT IN THE DICTIONARY onto the data stack 
LIT:
DD do_LIT
DB 0, 3, "LIT"
do_LIT:  
...
do_EQU0:
    ...
    NEXTC
 ;
_NE0:         ; v( n1 -- f1 )      return true if n1 is not equal to zero  ; VERIFIED
DD _EQU0
NE0:
DD do_NE0
DB 0, 3, "0<>"
do_NE0:
    ...
    NEXTC
;
_LT0:         ; ( n1 -- f1 )      return true if n1 is less than zero  ; VERIFIED
DD _NE0
LT0:
DD do_LT0
DB 0, 2, "0<"
do_LT0:
    ...
    NEXTC
 ;
_GT0:          ; ( n1 -- f1 )      return true (-1) if n1 is greater than zero  ; VERIFIED
DD _LT0
GT0:
DD do_GT0
DB 0, 2, "0>"
...
do_GTE:
    ...
    NEXTC
 ;
_ULT:        ;  ( u1 u2 -- f1 )   return true if unsigned u1 is less than  
DD _GTE
ULT:
DD do_ULT
DB 0, 2, "U<"
do_ULT:
    ...
    NEXTC
 ;
_UGT:         ; ( u1 u2 -- f1 )   return true if unsigned u1 is greater than
DD _ULT
UGT:
DD do_UGT
DB 0, 2, "U>"
do_UGT:
    ...
    NEXTC
 ;
_DULT:        ; ( ud1 ud2 -- f1 )   return true if unsigned double ud1 is
DD _UGT
DULT:
DD do_DULT
DB 0, 3, "DU<"
do_DULT:
    ...
    NEXTC
 ;
_UMIN:       ; ( u1 u2 -- n3 )   return the lesser of unsigned u1 and
DD _DULT
UMIN:
DD do_UMIN
DB 0, 4, "UMIN"
...
do_CDECR:
    ...
    NEXTC
 ;
_ON:      ; ( addr -- )  ;    set the contents of addr to ON ; (-1) VERIFIED 240123
DD CDECR
ON:
DD do_ON
DB 0, 2, "ON"
do_ON:
    ...
    NEXTC
 ;
_OFF:     ; ( addr -- )  ;    set the contents of addr of OFF ; (0)    VERIFIED 240123
DD _ON
OFF:
DD do_OFF
DB 0, 3, "OFF"
...
do_DOVAR: 
    ...
    ;JMP @DOV2
@DOV1:
    ADD EAX, 5      ;  ASSUMES CFA POINTER IS IN EAX
@DOV2:
    MOV ECX, 0
    MOV CL, BYTE PTR [EAX]
    ADD EAX, ECX
...
do_DOUSER: 
    ...
    NEXTC
; 
_DOCON:   ;( -- n1 )   ;  runtime for constants   VERIFIED 240109
DD _DOUSER  
DOCON:
DD do_DOCON
DB 0, 5, "DOCON"
do_DOCON: 
   ADD EAX, 5                  ; POINTER TO CFA IN EAX
   MOV EBX, 0
   MOV BL, BYTE PTR [EAX]
   ADD EAX, EBX
   INC EAX          
...
do_DOVALUE: 
   ...
   NEXTC
 ; 
_DOVALUESTORE:  ;( n1 -- )   ;  runtime for VALUE store    VERIFIED 240116
DD _DOVALUE
VALUESTORE:
DD do_VALUESTORE
DB 0, 8, "DOVALUE!"
do_VALUESTORE:         
    POP EAX                 ; CFA POPINTER
    ADD EAX, 5
    MOV EBX, 0
    MOV BL, BYTE PTR [EAX]
    ADD EAX, EBX
    ...
    NEXTC
 ; 
_DOVALPLUSTORE:  ;( n1 -- )   ;  runtime for VALUE increment   VERIFIED 240116
DD _DOVALUESTORE  
DOVALPLUSTORE:
DD do_VALPLUSTORE
DB 0, 9, "DOVALUE+!"
do_VALPLUSTORE: 
    POP EAX         ; DISCARD CFA POINTER
    POP EAX
    MOV EAX, 0
    MOV EBX, [NFA+9]
    MOV AL, BYTE PTR [EBX]
...
@@W1:
    ...
    add     edi, [S_IN+10]    	; add _IN
    mov     ecx, [S_LEN+11]     ; ecx = input length
    sub     ecx, [S_IN+10]      ; subtract _IN
    ja      short @@9A
...
@@7A:
    sub     edi, [S_ADR+11]     ; offset from start
    mov     [S_IN+10] , edi     ; update S_IN
...
do_PARSENAME:
    ...
    push    ebx
    mov     eax, [S_ADR+11]      ; edi = input pointer
    add     eax, [S_IN+10]   	 ; add _IN
...
@@4C:
    ...
    add     eax,  1    		 ; skip over ending delimiter
            ; update S_IN pointer and get word length
@@7C:
    sub     eax, [S_ADR+11]  ; offset from start
    mov     [S_IN+10] , eax  		 ; update S_IN
...
do_PARSE:
    ...
    POP     [S_ADR+11]              ; BUFFER TO SEARCH
    mov     eax, [S_ADR+11]         ; S_ADR = input pointer
    add     eax, [S_IN+10]  	    ; add S_IN
    mov     dl, BL      			; char to scan for eax = input char
    mov     ecx, [S_LEN+11]         ; ecx = input length
    sub     ecx, [S_IN+10]  		; subtract S_IN
    ja      short @@1D
    xor     ecx, ecx                ; at end of input
    push    eax
    jmp     short @@8D
...
NEXTC       ; LOWERCASE
;
_FIND: ; ( SADDR LEN -- POCKET cfa   ) Search dictionary for a word         VERIFIED
DD _LOWERCASE
FIND:
DD do_FIND
DB 0, 4, 'FIND'
...
@@1L:
 ...
 NEXTC
;
_POUND:              ; CODE #  ( d1 -- d2 ) \ convert a digit in pictured number output - see <#
DD _SPACES
DD do_POUND
DB 0, 1, '#'
do_POUND:
...
NEXTC
;
_itohex:                  ; (PAD N -- PAD)       VERIFIED 231206
DD _POUND
I2HEX:
DD do_I2HEX
DB 0, 5, 'I2HEX'
...
PUSH $ + 10
JMP do_DOCOL
DD DROP, LIT, 12, MSGOUT
DD POC, PLUS1, ZCOUNT, TYPE@
DD LIT, 13, MSGOUT, CR, DROP2
DD SEMI
;
COMMENT ~
...
; invoke ustr2dw, ustr2dw proc pszString:DWORD      ; Convert string to unsigned DWORD (EAX)
;
_DWTOA:  ; ( ADDR N -- )  CONVERT DWORD NUMBER TO ASCII STRING     VERIFIED 240109
DD _QUIT
DWTOA:
DD do_DWTOA
DB 0, 5, 'DWTOA'
...
NEXTC
;
_WDCOUNT:       ;  ( ENDPT, STARTPT -- N ) COUNT THE WORDS IN THE DICTIONARY       VERIFIED 240118
DD _ms          ; MUST USE THE ADDRESS OF THE LFA  EG.  ' PAD 4 -    
WDCOUNT:
DD do_WDCOUNT
DB 0, 7, 'WDCOUNT'
do_WDCOUNT:
POP EBX
SUB EBX, 4                    ;  POINTS TO LFA OF START
POP EDX
SUB EDX, 4                    ;  POINTS TO LFA OF END
MOV ECX, 0
@WDC1:
INC ECX
PUSH [EBX]
...
@WDC2:
PUSH ECX
NEXTC                         ; END OF WDCOUNT MOV [DCW+9], ECX
;
_?STACK:                ; QUERY STACK STATUS   VERIFIED 231201
DD _WDCOUNT
?STACK:
DD do_?STACK
DB 0, 6, '?STACK'
...
CMP ESP, EAX
JB @QS2
MOV [SERROR+12], 0           ; INBOUNDS
JMP do_?ERROR
@QS1:
MOV [SERROR+12], 23         ; stack underflow
JMP do_?ERROR
@QS2:
MOV [SERROR+12], 24         ; stack overflow
JMP do_?ERROR
;
_GCFA:          ; ( -- CFA)  PUT THE CFA ON THE STACK OF THE NEXT EXECUTABLE WORD    VERIFIED 231201
DD _?STACK      ; COMPILED CODE ONLY
GCFA:
DD do_GCFA
DB 0, 4, 'GCFA'
do_GCFA:
...
NEXTC
;
_GCFA@:          ; ( -- CFA)  PUT THE CFA ON THE STACK OF THE NEXT EXECUTABLE WORD    
DD _GCFA       ; COMPILED CODE ONLY
GCFA@:
DD do_GCFA@
DB 0, 4, '[GCFA]'
do_GCFA@:
...
NEXTC
;
_ALLOCATE:  ; ( N -- ADDR FL )     GLOBAL MEMORY ALLOCATION   VERIFIED 240110
DD _MALLOC
ALLOCATE:
DD do_ALLOCATE
DB 0, 8, 'ALLOCATE'
...
NEXTC
;
_free:          ; ( pMem -- ) release global memory allocated  VERIFIED 240110
DD _ALLOCATE 
FREE:
DD do_FREE
DB 0, 4, 'FREE'
...
NEXTC
;
_SCAN:          ; ( adr len char -- adr' len' ) search first occurence of char "char" in string
DD _ALIGN       ; VERIFIED 240114
SCAN:
DD do_SCAN
DB 0, 4, 'SCAN'
do_SCAN:
...
;
_MSGOUT:                    ; ( N -- )          VERIFIED 240115
DD _EOCD                    ; DISPLAY MESSAGE 
MSGOUT:
DD do_COLN 
DB 0, 6, 'MSGOUT'
DD GETMSGADR, @, ZCOUNT, TYPE@
DD SEMI
;
_DECIMAL:                   ; 10 BASE !       VERIFIED 240102
DD _MSGOUT
DECIMAL: 
DD do_COLN      ; do_DECIMAL
DB 0, 7, 'DECIMAL'
;do_DECIMAL:
;PUSH $ + 10
;JMP do_DOCOL 
...
PUSH $ + 10
JMP do_DOCOL 
DD STATE, @, IF@, S_ADR, @, ELSE@, TIB, THEN@
...
JMP do_DOCOL
DD STATE, @
DD IF@, S_ADR, @                                         ; DETERMINES WHERE INPUT IS FROM
DD ELSE@, TIB
DD THEN@
DD LIT, 32, WORD@, COUNT
DD DUP2, UPPERCASE, DROP
...
DD SEMI
;
_LITERAL:       ;( n -- ) COMPILATION WORD ONLY
...
NEXTC
;
_@UNTIL:         ; UNTIL ( F1 -- ) ?COMP  1 ?PAIRS  COMPILE _UNTIL  <RESOLVE ; IMMEDIATE
DD _UNTIL@ 
@UNTIL:
DD do_@UNTIL
DB 0, 5, 'UNTIL'
...
JMP do_DOCOL
DD PAD, LIT, 80, ERASE
DD PUSH1, ?STACK, ?ERROR, DROP
DD DEPTH, SMAX, MIN, SERROR, @, EQU0
DD IF@   ;  STACKERROR = 0
DD DUP@  ;  MIN = 0 
DD IF@
DD S0, @, LIT, 4, MINUS, TV3, STORE
DD LIT, 91, EMIT, DEPTH, MINUS1 ; DUP@                      ; do_DOTP, "["
...
DD LIT, 6, MSGOUT, CR                               ; Mempty, PUSH0, STDOUT       
DD THEN@ 
DD SERROR, @, PUSH0, NEQ                            ; STACKERROR <> 0
DD IF@
DD SERROR, @, MSGOUT, CR                            ; LIT, Msg22, PUSH0, STDOUT ; DISPLAY 'STACKERROR' MESSAGE
DD THEN@
DD SEMI
;
COMMENT ~
...
PUSH $ + 10
JMP do_DOCOL
DD TICK     ; LOCATE LFA OF WORD TO REMOVE
...
DD TYPE@
DD PUSH1, S_IN, PLUSSTORE    ; 1 S_IN @ + S_IN !     INCREMENT THE S_IN OFFSET
DD PAD, ZCOUNT, ERASE
DD SEMI
;
...
NEXTC
;
_ALLOT:         ; ( n -- ) \ allocate n bytes at HERE, increment DP         VERIFIED 240110
...
PUSH $ + 10
JMP do_DOCOL
DD TICK, NOOP, DUP@, EQU0, NOOP
DD IF@                             ; =0 THEN GET CFA OF NEXT COMPILED WORD
DD DROP, GCFA@                     ; CONVERT CFA TO NFA
DD THEN@
DD VALUESTORE
DD SEMI
;
_PTO:                        ; VERIFIED XXXXXX
DD _TO 
PTO:
DD do_PTO
DB 1, 3, '+TO'
...
DD LIT, 4
DD BEGIN@, TOR                              ; BEGIN2
DD BTOH, PAD, ZCOUNT, TYPE@, LIT, 32, EMIT
DD FROMR, MINUS1, DUP@  
DD UNTIL@                                   ; UNTIL2
DD DROP 
...
DD FROMR, MINUS1, DUP@  
DD UNTIL@                                   ; UNTIL1
DD DROP, SEP1, ZCOUNT, TYPE@
DD LIT, 125, PAUSE@
DD TV2, @, LIT, 16, MINUS, LIT, 16, CTYPE
DD SEP1, ZCOUNT, TYPE@, CR
DD FROMR, DUP@, TV2, @, MINUS, GT0
DD UNTIL@                                   ; UNTIL0
DD DROP, CR
DD SEMI
;
...
PUSH $ + 10
JMP do_DOCOL
DD LIT, 80000000H, FILEOPEN, SWAP, DUP@, GT0
DD IF@, FILECLOSE, DROP, ELSE@, NIP
DD THEN@
DD SEMI
;
...
;PUSH $ + 10
;JMP do_DOCOL
DD GCFA, S_ADR, LIT, 11, PLUS, @, LIT, 10, PARSE, DROP2              
DD SEMI
;
_COMMENT2:
DD _COMMENT1
...
;PUSH $ + 10
;JMP do_DOCOL
DD GCFA, S_ADR, LIT, 11, PLUS, @, LIT, 41, PARSE, DROP2 
DD SEMI
;
...
DD IF@, NIP, EXECUTE     
DD ELSE@, DROP, NUMBER, EMSG, THEN@         ; ? NUMBER AVAILABLE ON STACK    
DD SEMI
;
_BPA:    ; (  --  )
DD _INTERPRET
...
DB 0, 4, 'LOAD'
do_LOAD:
DD S_LEN, STORE, PUSH0, S_IN, STORE
DD S_ADR, STORE, STATE, ON, COMPF?, ON
DD BEGIN@
DD S_ADR, @, LIT, 32, WORD@
DD COUNT, DUP2, UPPERCASE, OVER, TYPE@, CR, FIND        
DD FFLAG, @ 
DD IF@, TOBPA
DD ELSE@, NUMBER@
DD THEN@
DD S_LEN, @, S_IN, @, MINUS        ; S_LEN @ S_IN @ - 
DD UNTIL@
DD STATE, OFF, COMPF?, OFF
DD CR, TIB, S_ADR, STORE
DD PUSH0, DUP@, S_LEN, STORE, S_IN, STORE
DD SEMI
;
_COLON:   
...
PUSH $ + 10			
JMP do_DOCOL						
DD MSGF2, PUSH0, RO, FILEOPEN, DROP, FH1, STORE
DD FH1, @, GFS, FS1, STORE
DD FS1, @, LIT, 128, PLUS, ALLOCATE
DD DROP, MSGB, STORE, MSGB, @, MSGBP, STORE
DD MSGB, @, FS1, @, ERASE 
DD MSGB, @, FS1, @, FH1,@, FILEREAD, DROP2
; process input file
DD TESTING  ; PROTOTYPE AREA
;
DD FH1, @, FILECLOSE, FH1, STORE
DD MSGB, @, FREE		
DD SEMI		
;
_TESTING:                                     ; USED TO PROTOTYPE NEW WORDS
DD _LOADFILE1
TESTING: 
DD do_TEST1
DB 0, 7, 'TESTING'
...
PUSH $ + 10
JMP do_DOCOL
DD FS1, @, S_LEN, STORE, PUSH0, S_IN, STORE
DD STATE, ON, NOOP
DD BEGIN@
DD MSGB, @, BLK, PARSE
DD DUP2, UPPERCASE, DROP, POC, DUP@ 
DD ZCOUNT, ERASE, DUP@, LIT, 2, PICK, SWAP, CSTORE 
DD PLUS1, OVER, SWAP, LIT, 3, PICK, SWAP, ROT 
DD CMOVE@, FIND
DD FFLAG, @, EQU0                                 
DD IF@, DROP, NUMBER, EMSG                   ; ? NUMBER, YES, ON STACK 
DD GCFA, EOD, LIT, 4, PLUS, NFA, STORE        ; LOAD NFA WITH A DUMMY POINTER
DD ELSE@, NIP
DD THEN@
DD NFA, @, MINUS1, C@, NE0
DD IF@                                       ; FLAG = TRUE <>0 THEN EXECUTE
DD EXECUTE
DD ELSE@                                     ; COMPILE CFA 
DD THEN@
DD S_IN, @, S_LEN, @, LT@ 
...
NEXTC  ;
;
_ML2:                        ; MOVE INTO ARRAY BEGINNING ADDRESS OF EACH MESSAGE LINE VERIFIED 240116
DD _TESTING
ML2: 
DD do_COLN  
DB 0, 3, 'ML2'
DD MSGB, @, LCT, @, PUSH1, PLUS, PUSH1                      ; MSGS BUFFER ADDRESS, # MESSAGES, 1 START POINT
DD BEGIN@
DD LIT, 2, PICK, LIT, 1, PICK, GETMSGADR                    ; 2 PICK 1 PICK 
DD STORE, PLUS1, ROT, ZCOUNT, PLUS, PLUS2                   ; ! 1+ ROT ZCOUNT + 2+
DD ROT, ROT, DUP2, MINUS
DD UNTIL@, DROP2, DROP
DD SEMI
;
_ML1:                        ; COUNT NUMBER OF LINES OF INPUT  VERIFIED 240116
DD _ML2
ML1:
DD do_COLN 
DB 0, 3, 'ML1'
DD MSGBP, @, FS1, @         ; LOCATE EOL CHAR 0DH WRITE WORD 0 TO END OF LINE 0D0AH -->  0000H
DD BEGIN@                   ; INCREMENT LCT POINTER
DD LIT, 13, SCAN, PUSH1, LCT, PLUSSTORE
DD OVER, PUSH0, SWAP, WSTORE, DUP@
DD UNTIL@
DD DROP2, PUSH1, LCT, MINUSSTORE
DD SEMI
;
_MSGLOAD:                   ; VERIFIED 240116
DD _ML1
MSGLOAD: 
DD do_COLN 
DB 0, 7, 'MSGLOAD'
DD MSGF1, PUSH0, RO, FILEOPEN, DROP, FH1, STORE
DD FH1, @, GFS, FS1, STORE
DD FS1, @, LIT, 128, PLUS, ALLOCATE
DD DROP, MSGB, STORE, MSGB, @, MSGBP, STORE
DD MSGB, @, FS1, @, ERASE 
...
DD _MOD
DIVIDE: 
DD do_COLN       ;            do_DIVIDE
DB 0, 1, '/'
DD MODIVIDE, NIP
DD SEMI
;
_SP:                            ; ( -- ADDR N)  EG S" TEXT TO BE HANDLED"    VERIFIED 240110
DD _DIVIDE
SP@:
DD do_COLN                      ; do_SP
...
DD TIB, LIT, '"', WORD@                       ; PARSE INPUT STRING TIB " WORD
DD COUNT                                      ; GET LENGTH
DD PUSH1, S_IN, PLUSSTORE                      ; 1 S_IN @ + S_IN !     INCREMENT THE _IN OFFSET
...
DD SEMI
;
_ABT1:               ; USED IN NORMAL STARTUP       VERIFIED 240118
DD _SP
ABT1:
DD do_COLN 
DB 0, 4, 'ABT1'
DD GCFA, ENDOFLINE, GCFA, EOCD, WDCOUNT         ; COUNT WORDS IN CORE
DD GCFA, EOCD, GCFA, SOSV, WDCOUNT              ; COUNT # OF COMPILED WORDS
DD GCFA, SOSV, GCFA, EOD, WDCOUNT               ; COUNT # OF VARIABLES...
DD GCFA, ENDOFLINE, GCFA, EOD, WDCOUNT          ; COUNT TOTAL # OF WORDS IN DICTIONARY
DD DCW, STORE
DD LIT, 7, LIT, 22, XYLOCATE
DD PAD, DCW, @
DD DWTOA, PAD, PUSH0, STDOUT
DD LIT, 8, LIT, 31, XYLOCATE
DD PAD, SWAP, DWTOA, PAD, PUSH0, STDOUT
DD LIT, 9, LIT, 13, XYLOCATE
DD PAD, SWAP, MINUS1, DWTOA, PAD, PUSH0, STDOUT
DD LIT, 10, LIT, 17, XYLOCATE
DD PAD, SWAP, MINUS1, DWTOA, PAD, PUSH0, STDOUT
DD LIT, 11, PUSH0, XYLOCATE
DD CRLF, PUSH0, STDOUT
DD CRLF, PUSH0, STDOUT
DD PUSH0, ABFLG, STORE
DD FB1, LIT, 1024, ERASE
DD SEMI
;
...
MOV [S_IN+10], 0          ; RESET IN BUFFER OFFSET
...
DD do_COLN 
DB 0, 5, '>MSG{'
DD LIT, 4, MULT, MSGS, PLUS 
DD SEMI
;
_?ERROR:                  ; ( N -- )  CHECK FOR ERROR AND DISPLAY MESSAGE                  
DD _MSGADR
?ERROR: 
DD do_?ERROR
DB 0, 6, '?ERROR'
...
Msg9 db ' Fig Forth Beginning', 0h
...
;
MsgQ db ' ? ', 0H
MsgF1 db 'ERMSGS.TXT', 0H
MsgF2 db 'LIB-BUILD1.F', 0h
SEP1$ db ' | ', 0h
SPACE$ db 20H,0H
...
DD 0
;
_LATEST DD _SOSV                           ; Returns address of the variable LATEST           ; VERIFIED
...
; ********************
;
_FB1 DD _FS1     ; FILE 1 BUFFER
FB1 DD do_DOVAR
DB 0, 3, 'FB1'
DB 1024 DUP (0H)
;
;
_PAD DD _FB1     ; OUTPUT BUFFER
PAD DD do_DOVAR
DB 0, 3, 'PAD'
DB 256 DUP (0H)
;
...
DB 256 DUP (0H) 
;
_MSGS DD _POC           ; MESSAGE ARRAY 30 ELEMENTS SIZE CELL (4)
MSGS DD do_DOVAR
DB 0, 4, 'MSG{'
DD 32 DUP (0H)
;
