...
4 PROC getaddrinfo  ( char* nodename, char* servname, struct addrinfo* hints -- struct addrinfo** res )
...
: sock-open ( addr len port -- sock flag )  \ Change made by FJR to return flag 071025
...
\ ** If the socket cannot be opened, a exception will be thrown.
  htons saddr 2 + w!
  AF_INET saddr w!
  host>iaddr dup saddr 4 + !
  if
       0 SOCK_STREAM AF_INET socket dup
       16 saddr rot connect
  else 0 -1  \ unable to look up uphost
  then
  ;
